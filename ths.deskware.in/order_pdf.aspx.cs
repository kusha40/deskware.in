...
public partial class order_pdf : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (Request.QueryString["id"] != null )
            {
                ...
                string query6 = "select * from tbl_order_details where order_no='" + q_no + "' ";
               ...
               // rpt.SetDatabaseLogon("db_indusuno", "Wqp0h~32", "182.50.133.109", "db_indusuno");
                string PDFFile = Server.MapPath("Order/" + q_no + "-" + DateTime.Now.ToString("dd.MM.yy-hh.mm.ss.tt") + ".pdf");
                path = "Order/" + q_no + "-" + DateTime.Now.ToString("dd.MM.yy-hh.mm.ss.tt") + ".pdf";
                rpt.ExportToDisk(ExportFormatType.PortableDocFormat, PDFFile);

                //string order_no, lead_id, name, mob, email, product, gtotal, advance, balance = "";
                string query1 = "select * from tbl_order_details where order_no='" + q_no + "'";
                ...
                con.Open();
                cmd5.CommandText = "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '12:40:00';declare @date DATETIME;set @date=(SELECT (@d + CAST(@t AS datetime))) insert into tbl_order_history values('" + q_no + "','" + lead_id + "','" + name + "','" + mob + "','" + email + "','" + product + "','" + gtotal + "','" + advance + "','" + balance + "','" + Session["Username"].ToString() + "',@date,'" + path + "')";
                cmd5.ExecuteNonQuery();
                con.Close();

                sendmail();
                rpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, "Order");
                Response.End();
            }
        }
    }
