   ...
   SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["connect"].ToString());
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                cls.gridbind(GridView1, "Select * from tbl_order_history  order by date desc");
            }
            else
            {
                cls.gridbind(GridView1, "Select * from tbl_order_history where  created_by in (select uid from tbl_team where aid='" + Session["Username"].ToString() + "')   order by date desc");
            }
        }
    ...
    protected void btnview2_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                cls.gridbind(GridView1, "Select * from tbl_order_history where lead_id='" + arr_search[0].Trim() + "'  order by date desc");
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        con.Open();
        SqlCommand cmd = new SqlCommand("delete FROM tbl_order_history where id='" + Convert.ToInt32(GridView1.DataKeys[e.RowIndex].Value.ToString()) + "'", con);
        cmd.ExecuteNonQuery();
        con.Close();
