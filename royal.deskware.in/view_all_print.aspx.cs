...
public partial class view_all_print : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
                ...
                //{
                //    cls.datalist(DataList1, " DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '12:30:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select distinct date, challan_no,dealer_name,dealer_area,totlamount,fraight,netamount,inwords,created_by,status,type from tbl_challan_details_master where date=@date_new and dealer_id='" + Request.QueryString["id"].ToString() + "'  order by challan_no asc");
