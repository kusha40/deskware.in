        ...
        if (ds.Tables[0].Rows.Count != 0)
        {
            ...
            for (j = 0; j < GridView1.Rows.Count; j++)
            {
                ...
                if (cb.Checked == true)
                {
                    ...
                    con.Open();
                    cmd.CommandText = "Delete tbl_dealer_ledger_master    where id='" + id.Text + "' ";
                    cmd.ExecuteNonQuery();
                    con.Close();
            ...
            if (count > 0)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "AlertBox", "alert('Deleted Sucessfully');window.location='dealer_ledger.aspx';", true);
            }
            else
            {
