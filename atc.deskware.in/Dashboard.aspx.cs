...
public partial class Dashboard : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (user_type == "Administrator")
            {
                tbl1.Visible = true;
                //and date=@date_new 
                cls.gridbind(GridView2, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_dealer_discount_approve_details where status='Not Approved' and discount!='0.00'  order by date asc");
                ...
                cls.gridbind(GridView1, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_permission_expense_Approval where status='Not Approved' and amount!='0.00'   order by date asc");

                cls.gridbind(GridView3, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_checking_expense_Approval where status='Not Approved' and amount!='0.00'   order by date asc");
                img1.Visible = true;
                Panel2.Visible = true;
            }
            else
            ...
            if (Session["Username"].ToString() == "amit001")
            {
                Panel2.Visible = true;
            }
            ...
            if (Session["Username"].ToString() == "amit-pat")
            {
                img1.Visible = false;
                tbl1.Visible = true;
                pnl.Visible = Panel2.Visible = true;
                cls.gridbind(GridView2, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_dealer_discount_approve_details where status='Not Approved' and discount!='0.00'  order by date asc");
   ...
   //    string query = "";
   //    string name = (string)HttpContext.Current.Session["Username"];
   //    query = string.Format("DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101)  SELECT  'Today-'+Cast(Count(todolist_master.id)As varchar)as qty,Count(todolist_master.id)as totl FROM todolist_master where  todolist_master.date=@date_new and todolist_master.status='Pending' and todolist_master.user_id='" + name + "' union all   SELECT  'Pending-'+Cast(Count(todolist_master.id)As varchar) as qty,Count(todolist_master.id)as totl FROM todolist_master where  datediff(day,todolist_master.date,@date_new)>0 and todolist_master.status='Pending' and todolist_master.user_id='" + name + "'");
   ...
   //}
    ...
    {
        tbl1.Visible = true;
        cls.gridbind(GridView2, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_dealer_discount_approve_details where status='Not Approved' and discount!='0.00' and dealer_id='" + ddldealer.SelectedValue + "' and date between '" + DateTime.ParseExact(txtfrom.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture).ToString("MM/dd/yyyy", CultureInfo.InvariantCulture) + "' and '" + DateTime.ParseExact(txtto.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture).ToString("MM/dd/yyyy", CultureInfo.InvariantCulture) + "'  order by date asc");
    ...
    {
        tbl1.Visible = true;
        cls.gridbind(GridView1, "DECLARE @d DATETIME, @t TIME;set @d = getdate();SET @t = '00:00:00';declare @date DATETIME;declare @date_new datetime;set @date=(SELECT (@d + CAST(@t AS datetime)))set @date_new=CONVERT(nvarchar(30), @date, 101) select * from tbl_permission_expense_Approval where status='Not Approved' and amount!='0.00' and date between '" + DateTime.ParseExact(TextBox1.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture).ToString("MM/dd/yyyy", CultureInfo.InvariantCulture) + "' and '" + DateTime.ParseExact(TextBox2.Text, "MM/dd/yyyy", CultureInfo.InvariantCulture).ToString("MM/dd/yyyy", CultureInfo.InvariantCulture) + "'  order by date asc");
    }
